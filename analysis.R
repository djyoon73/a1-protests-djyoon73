...
library(stringr)
num_in_wa <- sum(stringr::str_detect(locations, "WA"), na.rm = TRUE)
...
count_in_location <- function(location){
  n <- sum(stringr::str_detect(locations, location), na.rm = TRUE)
  return(cat("There were", 
             n,
             "protests in ",
             location,
             "."))
...
# `stringr` package
states <- c(stringr::str_sub(locations, -2, -1))

# Create a vector of the unique states in your dataset. `uniq_states`
uniq_states <- unique(states, na.rm = TRUE)
...
state_summary <- lapply(uniq_states, count_in_location)

# Create a summary table by passing your `states` variable to the `table()`
# functon, and storing the result in a variable `state_table`.
...
in_2020 <- stringr::str_subset(dates, "2020")

# Create a vector of the dates that are in 2019. `in_2019`
in_2019 <- stringr::str_subset(dates, "2019")
...
## It does not surprise me that the number of protests in 2020 nearly doubled
## the number of protests in 2019. In 2020, there were more incidents that 
## triggered social unrest, such as George Floyd's death in May of 2020. This 
## led to many protests regarding police brutality and racial injustice. 
...
months_in_2020 <- months(as.Date(in_2020))
by_month_table <- table(months_in_2020)
View(by_month_table)
...
months_in_2019 <- months(as.Date(in_2019))
by_month_table_2019 <- table(months_in_2019)
View(by_month_table_2019)

number_July_2019 <- sum(stringr::str_detect(months_in_2019, "July"))
number_July_2020 <- sum(stringr::str_detect(months_in_2020, "July"))
change_july_protests <- abs(number_July_2020 - number_July_2019)
...
high_level_purpose <- unlist(strsplit(purpose, "\\(.*\\)"))
...
attendees_at_protest <- function(attendees){
  x <- sum(stringr::str_detect(protests$Attendees, attendees), na.rm = TRUE)
  return(cat("There were", 
             x,
             "protests with",
             attendees,
